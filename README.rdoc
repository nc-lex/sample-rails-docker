...
* Exit all containers, and run <code>docker-compose build</code> after <code>Gemfile</code> is updated.
...
* The entrypoint script will automatically initialize the Rails database. Deleting the <code>tmp/init</code> file forces it to initialize again.
