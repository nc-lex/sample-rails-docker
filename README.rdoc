...
* The Docker container may have a different timezone. Use <code>.docker/start -t</code> to sync the timezone. (sudo required)
...
* <code>docker-compose build</code> can be run routinely to rebuild the Docker image and recreate the containers to clean them up.
