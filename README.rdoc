...
In the App Server project three Docker images are built, the minimal one for essential and stable packages, another for the mongo server with dump files restored, and the last for development with all required packages installed. As long as the minimal one is not updated, building the complete one will be much faster than building from scratch.
